...
namespace wanshu_api_demo
{
	...
	public class YingYeZhiZhaoApi
	{
	    ...
	    const string API_URL_KONGHAO = "https://api.253.com/open/company/business-license";
	    ...
	    public void Check() {
	        ...
	        if (jsonObject != null) {
	            ...
	            if ("200000".Equals(code) && null != jsonObject["data"]) {
                	...
                	Console.WriteLine("企业营业执行查询成功,content is :" + content);
	            } else {
	                // 记录错误日志，正式项目中请换成log打印
	                Console.WriteLine("企业营业执行查询失败,code:" + code + ",msg:" + jsonObject["message"]);
	            }
	        }
	    }
