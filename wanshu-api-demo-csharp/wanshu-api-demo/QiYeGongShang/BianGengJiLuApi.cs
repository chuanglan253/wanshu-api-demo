...
namespace wanshu_api_demo
{
	...
	public class BianGengJiLuApi
	{
	    ...
	    const string API_URL = "https://api.253.com/open/company/change-record";
	    ...
	    public void Check() {
	        ...
	        if (jsonObject != null) {
	            ...
	            if ("200000".Equals(code) && null != jsonObject["data"]) {
	                ...
	                Console.WriteLine("公司变更记录信息查询成功，分页信息:"+pageInfoStr);
                	String content = jsonObject["data"]["datas"].ToString();
                	Console.WriteLine("公司变更记录信息查询成功,content is :" + content);
	            } else {
	                // 记录错误日志，正式项目中请换成log打印
	                Console.WriteLine("公司变更记录查询失败,code:" + code + ",msg:" + jsonObject["message"]);
	            }
	        }
	    }
