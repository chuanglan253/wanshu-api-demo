 ...
 * 分支机构查询
 */
public class FenZhiJiGouApi {

	private static String APP_ID = "qqqqqqqq";

    private static String APP_KEY = "qqqqqqqq";

    private static String API_URL = "https://api.253.com/open/company/branch";

    private static JsonParser jsonParser = new JsonParser();

	public static void main(String[] args) {
        // 1.调用api
		JsonObject jsonObject = invokeBranch();
    	// 2.处理返回结果
        if (jsonObject != null) {
            ...
            if ("200000".equals(code) && jsonObject.get("data") != null) {
            	...
            	System.out.println("公司分支机构信息查询成功，分页信息:"+pageInfoStr);
                String content = jsonObject.get("data").getAsJsonObject().get("datas").toString();
                System.out.println("公司分支机构信息查询成功,content is :" + content);
            } else {
                // 记录错误日志，正式项目中请换成log打印
                System.out.println("公司分支机构信息查询失败,code:" + code + ",msg:" + jsonObject.get("message").getAsString());
            }
        }

	}
	
	private static JsonObject invokeBranch() {
