...
public class KonghaoPiLiangApi {
    ...
    public static void main(String[] args) {
        // 1.调用空号检测api
        final JsonObject jsonObject = invokeKonghao("1999999***,18888888***");
        ...
        if (jsonObject != null) {
            ...
            if ("200000".equals(code) && jsonObject.get("data") != null) {
                ...
                // 解析结果数据，进行业务处理
                // 检测结果
                System.out.println("调用空号批量检测成功,data:" + jsonObject.get("data"));
            } else {
                // 记录错误日志，正式项目中请换成log打印
                System.out.println("调用空号批量检测失败,code:" + code + ",msg:" + jsonObject.get("message").getAsString());
            }
        }
    }


    private static JsonObject invokeKonghao(String mobiles) {
