...
public class NianBaoLieBiaoApi {
    ...
    private static String API_URL = "https://api.253.com/open/company/reportlist";

    private static JsonParser jsonParser = new JsonParser();

	public static void main(String[] args) {
        // 1.调用api
		JsonObject jsonObject = invokeReportlist();
    	// 2.处理返回结果
        if (jsonObject != null) {
            ...
            if ("200000".equals(code) && jsonObject.get("data") != null) {
                ...
                System.out.println("公司年报列表信息查询成功,content is :" + content);
            } else {
                // 记录错误日志，正式项目中请换成log打印
                System.out.println("公司年报列表信息查询失败,code:" + code + ",msg:" + jsonObject.get("message").getAsString());
            }
        }

	}
	
	private static JsonObject invokeReportlist() {
